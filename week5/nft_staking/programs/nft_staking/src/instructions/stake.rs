...
pub fn handler(ctx: Context<Stake>) -> Result<()> {
    ...
    // Loop to all attributes and save only the ones that are not the staking attributes("staked", "staked_at", "last_claimed")
    ...
    if let Some(attributes) = &attributes_fetched {
        for attribute in &attributes.attribute_list {
            ...
            else if attribute.key != "staked_at" && attribute.key != "last_claimed" {
    ...
    // Now that we have the complete list of attributes we either add the plugin or update the exisiting one
    // The Attributes Plugin is an Authority-Managed Plugin, so it needs to be signed by the update authority(PDA of the prog)
    ...
    let collection_attrs_fetched: Option<Attributes> = fetch_plugin::<BaseCollectionV1, Attributes>(
    ...
    // Preserve existing collection attributes, extract current staked_count
    if let Some(attrs) = &collection_attrs_fetched {
    ...
    staked_count = staked_count.checked_add(1).ok_or(ErrorCode::Overflow)?;
