...
pub struct CreateCollection<'info> {
    ...
    /// CHECK: this account is not initialized and is being used for signing purposes only, we verify that it is derived from correct seeds
    #[account(
...
pub fn handler(ctx: Context<CreateCollection>, name: String, uri: String) -> Result<()> {
    // get signer seeds for the update authority
